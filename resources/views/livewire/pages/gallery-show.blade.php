...
>
    ...
    <div x-show="open" x-transition.opacity.duration.300ms @click.self="open = false"
         class="fixed inset-0 h-screen w-screen bg-black/90 flex items-center justify-center z-50">
        <button class="absolute top-5 right-5 text-white text-3xl hover:text-ritz-gold" @click="open = false">&times;</button>
        <button class="absolute left-4 text-5xl text-white hover:text-ritz-gold px-4 transition" @click="prev()">‹</button>

        <img :src="images[currentImage]"
             class="max-h-[90vh] max-w-full mx-auto rounded-lg shadow-2xl object-contain transition duration-300">

        <button class="absolute right-4 text-5xl text-white hover:text-ritz-gold px-4 transition" @click="next()">›</button>
    </div>
</div>
